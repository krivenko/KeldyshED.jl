...
This constructor uses the [`autopartition procedure`](@ref "Space autopartition algorithm"),
and the QR algorithm to diagonalize the blocks. The invariant subspaces of the Hamiltonian
are chosen such that all creation and annihilation operators carrying compound indices
