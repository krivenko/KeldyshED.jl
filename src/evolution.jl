...
Compute the partition function ``Z = \\mathrm{Tr}[e^{-\\beta\\hat H}]``
at an inverse temperature ``\\beta``.
"""
function partition_function(ed::EDCore, β::Real)::Real
...
Compute the equilibrium density matrix ``\\hat\\rho = e^{-\\beta\\hat H} / Z`` at an
inverse temperature ``\\beta`` represented in the eigenbasis of the Hamiltonian ``\\hat H``.
The density matrix is returned as a vector of diagonal blocks corresponding to invariant
subspaces of ``\\hat H``.
"""
...
```
on a given contour time grid and represented in the eigenbasis of the Hamiltonian
``\\hat H``.
The operator is returned as a vector of diagonal blocks (matrix-valued Green's function
containers) corresponding to invariant subspaces of ``\\hat H``.
"""
...
Transform a block-diagonal evolution operator `S` written in the eigenbasis of the system
...
Transform a block-diagonal evolution operator `S` written in the Fock state basis into
...
The matrix is expected to be written in the Fock state basis. The resulting reduced matrix
acts in a [`Hilbert space`](@ref KeldyshED.Hilbert.FullHilbertSpace) spanned by all
fermionic Fock states generated by the
[`set of indices`](@ref KeldyshED.Hilbert.SetOfIndices) `target_soi`.
"""
...
Compute a partial trace of a block-diagonal evolution operator `S` written in
the Fock state basis.

The resulting reduced evolution operator acts in a
[`Hilbert space`](@ref KeldyshED.Hilbert.FullHilbertSpace) spanned by
all fermionic Fock states generated by the
[`set of indices`](@ref KeldyshED.Hilbert.SetOfIndices) `target_soi`.
"""
...
The Hamiltonian ``\\hat H`` acts in a direct product of
[`Hilbert spaces`](@ref KeldyshED.Hilbert.FullHilbertSpace)
``H_{target} \\otimes H_{env}``, while the resulting reduced matrix acts
in ``H_{target}`` spanned by all fermionic Fock states generated by the
[`set of indices`](@ref KeldyshED.Hilbert.SetOfIndices) `target_soi`.
"""
...
The Hamiltonian ``\\hat H`` acts in a direct product of
[`Hilbert spaces`](@ref KeldyshED.Hilbert.FullHilbertSpace)
``H_{target} \\otimes H_{env}``, while the resulting reduced evolution operator
acts in ``H_{target}`` spanned by all fermionic Fock states generated by
the [`set of indices`](@ref KeldyshED.Hilbert.SetOfIndices) `target_soi`.
"""
